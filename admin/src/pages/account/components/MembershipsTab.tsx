...
export function MembershipsTab({ cv, onUpdate, isEditing }: { cv: MemberCV; onUpdate: () => void; isEditing?: boolean }) {
  const { token } = useAuth();
  const [adding, setAdding] = useState(false);
  const [saving, setSaving] = useState(false);
  const [deletingId, setDeletingId] = useState<string | null>(null);
  
  const [form, setForm] = useState({ organization_name: "" });
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.me.cv.create(token, "memberships", form);
      setForm({ organization_name: "" });
      setAdding(false);
      onUpdate();
    } finally {
      setSaving(false);
  ...
  const handleDelete = async (id: string) => {
    if (!token || !confirm("Delete this membership?")) return;
    setDeletingId(id);
    try {
      await api.me.cv.delete(token, "memberships", id);
      onUpdate();
    } finally {
      setDeletingId(null);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-zinc-900 font-mono tracking-tight underline decoration-zinc-100 underline-offset-8">Memberships</h2>
        {isEditing && (
      ...
      {isEditing && adding && (
        <form onSubmit={handleAdd} className="bg-white border border-dashed border-zinc-300 p-5 rounded-xl space-y-4 animate-in zoom-in-95 duration-200">
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest block">Organization / Society Name</label>
            <input required value={form.organization_name} onChange={(e) => setForm({ ...form, organization_name: e.target.value })} className="w-full px-3 py-2 bg-zinc-50 border border-zinc-200 rounded-lg text-sm focus:ring-0 focus:border-zinc-400" placeholder="e.g. IEEE Computer Society" />
          </div>
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={saving} className="flex items-center gap-2 px-4 py-1.5 bg-black text-white text-[10px] font-bold uppercase tracking-widest rounded-lg shadow-md active:scale-95 transition-all">
              {saving ? <Loader2 size={12} className="animate-spin" /> : "Save Membership"}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="space-y-3">
        {cv.memberships?.length === 0 && !adding && (
          <div className="p-12 text-center border border-dashed border-zinc-200 rounded-2xl bg-zinc-50/50">
            ...
            {isEditing && <p className="text-[10px] text-zinc-400 uppercase mt-1">Click 'Add Membership' to begin</p>}
          </div>
        )}
        
        {cv.memberships?.map((item) => (
          <div key={item.id} className="group bg-white border border-zinc-200 p-4 rounded-xl flex items-center justify-between hover:border-zinc-400 transition-all shadow-sm">
            <div className="flex items-start gap-3">
              <Users className="text-zinc-400 mt-1 flex-shrink-0" size={16} />
              <p className="text-sm font-medium text-zinc-900 tracking-tight leading-relaxed">{item.organization_name}</p>
            </div>
            {isEditing && (
